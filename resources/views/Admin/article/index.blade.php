...
@section('content')
    <div class="row">
        <div class="col-12">
            <h1>فهرست مقاله ها  </h1>
            <nav class="breadcrumb-container d-none d-sm-block d-lg-inline-block" aria-label="breadcrumb">
                <ol class="breadcrumb pt-0">
                    <li class="breadcrumb-item active">فهرست</li>
                    <li class="breadcrumb-item" aria-current="page"><a href="{{route('article.create')}}">جدید</a>
                    </li>

                </ol>
            </nav>
            <a href="{{ route('article.create') }}" class="btn-sm btn-primary m-4">ساخت پست</a>
            ...
            <div class="card mb-4">
                <div class="card-body">
                    <table id="dataTable" class="table table-bordered table-hover">
                        <thead>
                        <tr>
                            <th style="white-space: pre">نویسنده</th>
                            <th style="white-space: pre">دسته بندی</th>
                        ...
                        @foreach($articles as $article)
                            <tr>
                                <td>{{$article->userRelation->name}}</td>
                                <td>{{$article->categoryRelation->title}}</td>
                                <td>{{$article->title}}</td>
                                <td>{{$isActive[$article->statusRelation->is_active]}}</td>
                                <td><a class='fa-edit mx-1 fa text-primary h4'
                                       href="{{route('article.edit',$article->id)}}"></a>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
